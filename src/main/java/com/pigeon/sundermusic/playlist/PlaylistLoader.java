...
package com.pigeon.sundermusic.playlist;

import com.pigeon.sundermusic.BotConfig;
                ...
                manager.loadItemOrdered(name, items.get(i), new AudioLoadResultHandler() 
                {
                    ...
                    public void trackLoaded(AudioTrack at) 
                    {
                        if(config.isTooLong(at))
                            errors.add(new PlaylistLoadError(index, items.get(index), "Этот трек длиннее допустимого максимума"));
                        else
                        {
                            at.setUserData(0L);
                            tracks.add(at);
                    ...
                    {
                        errors.add(new PlaylistLoadError(index, items.get(index), "Не удалось загрузить трек: "+fe.getLocalizedMessage()));
                        done();
                    }
                });
            }
