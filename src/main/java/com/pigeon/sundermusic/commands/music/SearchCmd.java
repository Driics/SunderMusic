...
public class SearchCmd extends MusicCommand 
{
    ...
    public SearchCmd(Bot bot)
    {
        ...
        this.help = "ищет на YouTube по указаному запоросу";
    ...
    private class ResultHandler implements AudioLoadResultHandler 
    {
        ...
        public void trackLoaded(AudioTrack track)
        {
            ...
            m.editMessage(FormatUtil.filter(event.getClient().getSuccess()+" Добавлен трек **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0 ? "для проигрывания"
                        : " в очередь на позицию "+pos))).queue();
        }
        ...
        public void playlistLoaded(AudioPlaylist playlist)
        {
            builder.setColor(event.getSelfMember().getColor())
                    ...
                    .setSelection((msg,i) -> 
                    {
                        ...
                        if(bot.getConfig().isTooLong(track))
                        {
                            event.replyWarning("Этот трек (**"+track.getInfo().title+"**) длиннее допустимого максимума: `"
                                    +FormatUtil.formatTime(track.getDuration())+"` > `"+bot.getConfig().getMaxTime()+"`");
                            return;
                        }
                        AudioHandler handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
                        int pos = handler.addTrack(new QueuedTrack(track, event.getAuthor()))+1;
                        event.replySuccess("Добавлен трек **"+track.getInfo().title
                                +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0 ? "для проигрывания"
                                    : " в очередь на позицию "+pos));
                    })
                    .setCancel((msg) -> {})
        ...
        {
            m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" Результаты для `"+event.getArgs()+"` не найдены.")).queue();
        }
